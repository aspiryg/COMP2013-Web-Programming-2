 ...
 */
//WRITE YOUR CODE BELOW
const listing1 = { ...listing0, isSold: false, currentOwner: "Jane Doe" };
...
const convertToNumber = (text) => {
  return text ? Number(text.substring(1).replace(",", "")) : 0;
};

const realtorFees = (listing) => {
  let fees = 0;
  fees =
    convertToNumber(listing.price) > 450000
      ? convertToNumber(listing.price) * 0.02
      : convertToNumber(listing.price) * 0.025;
...
// Since the order by field is not mentioned, I will sort the array based on listing price.
const listingAscendingly = [...listings].sort(
  (listingA, listingB) =>
    convertToNumber(listingA.price) - convertToNumber(listingB.price)
);
...
// Since the builtIn property is a string and some of them have text like Renovated 2009, I will extract the year from the string and then filter the listings.
// console.log(listings.map((listing) => listing.propertySummary.builtIn));

//
const exractNumberFromText = (str) => {
  return Number(str.match(/\d+/g)) ? Number(str.match(/\d+/g)[0]) : 0;
};

const newBuiltListings = listings.filter(
  (listing) => exractNumberFromText(listing?.propertySummary?.builtIn) >= 1990
);
